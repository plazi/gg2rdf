...
docker run -ti -v "$GITHUB_WORKSPACE"/xml:/workspace/xml -v "$GITHUB_WORKSPACE"/ttl:/workspace/ttl -v "$GITHUB_WORKSPACE"/gg2rdf.xslt:/workspace/gg2rdf.xslt -v "$GITHUB_WORKSPACE"/main-on-all.sh:/workspace/main-on-all.sh ubuntu /workspace/main-on-all.sh
